...
def calculate_fv(mags, dirs, bins):
    ...
    for i in range(bins):
        ...
        if i != bins - 1:
            hist[i] += None  # YOUR CODE HERE
            hist[i + 1] += None  # YOUR CODE HERE
        else:
            hist[i] += None  # YOUR CODE HERE
            hist[0] += None  # YOUR CODE HERE
...
def HOG(img, img_size=(64, 128), cell_size=(8, 8), block_size=(2, 2), orientations=9):
    ...
    type: int, default 9
    :return:
    fv: The final feature vector for the image
    gx: derivatives along horizontal axis
    gy: derivatives along vertical axis
    """
        ...
        for j in range(0, img_size[0] - x_stride * (block_size[0] - 1), x_stride):
            ...
            for _ in range(block_size[0]):
                for _a_ in range(block_size[1]):
                    ...
                    g_dir_c = None  # YOUR CODE HERE
                    g_mags_c = None  # YOUR CODE HERE
                    i = i + y_stride
                    j = j + x_stride
                    cell_v.extend(calculate_fv(g_dir_c, g_mags_c, orientations))
            # Can you optimize this algorithm?
            block_v = np.array(cell_v).ravel()
            # Normalize block_v
            block_v = None  # YOUR CODE HERE
            fv.extend(block_v)
